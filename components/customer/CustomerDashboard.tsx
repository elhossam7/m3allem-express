...
import { Customer, JobRequest, JobStatus, Notification, Artisan } from '../../types';
...
import { getJobById, getArtisansByIds } from '../../services/api';
...
const CustomerDashboard: React.FC<CustomerDashboardProps> = ({ user, notificationLink, onLinkConsumed }) => {
  ...
  if (selectedJob) {
    // Debug logging
    console.log('CustomerDashboard rendering JobStatusView with onReview:', !!setReviewingJob);
    ...
    return (
      <JobStatusView
        ...
        onReview={(job: JobRequest) => {
          console.log('onReview called with job:', job.id);
          setReviewingJob(job);
        }}
      />
    );
