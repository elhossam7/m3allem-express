...
const JobStatusView: React.FC<JobStatusViewProps> = ({ jobRequest: initialJobRequest, onBack, onRefreshJob, onReview }) => {
  ...
  const handleAcceptBid = async (bid: Bid, artisan: Artisan) => {
    try {
      ...
      // Set all required state for the modal at once to prevent race conditions
                ...
                {jobRequest.status === JobStatus.Completed && (
                    <div className="mt-6 pt-4 border-t space-y-3">
                        <div className="flex flex-col sm:flex-row gap-3 justify-center">
                            {/* Always show review button if conditions are met */}
                            {!jobRequest.isReviewed && onReview && (jobRequest.paymentStatus === 'released' || jobRequest.paymentStatus === 'paid') && (
                                <button 
                                    onClick={() => onReview(jobRequest)}
                                    className="bg-cyan-500 text-white font-bold py-2 px-6 rounded-lg hover:bg-cyan-600 transition duration-300 shadow-md flex items-center justify-center gap-2"
                                >
                            ...
                            {/* Debugging info - shows for all completed jobs */}
                            {jobRequest.status === JobStatus.Completed && (
                                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                                    <p className="font-semibold text-blue-800">Debug Info:</p>
                                    <p>Job ID: {jobRequest.id}</p>
                                    <p>Is Reviewed: {jobRequest.isReviewed ? 'Yes' : 'No'}</p>
                                    <p>Payment Status: {jobRequest.paymentStatus}</p>
                                    <p>OnReview Handler: {onReview ? 'Available' : 'Missing'}</p>
                                    <p>Review Button Should Show: {
                                        (!jobRequest.isReviewed && onReview && (jobRequest.paymentStatus === 'released' || jobRequest.paymentStatus === 'paid')) ? 'YES' : 'NO'
                                    }</p>
                                </div>
                            )}
                            
                            {/* Fallback review button for testing - only shows if main button doesn't */}
                            {jobRequest.status === JobStatus.Completed && 
                             !(!jobRequest.isReviewed && onReview && (jobRequest.paymentStatus === 'released' || jobRequest.paymentStatus === 'paid')) && 
                             onReview && (
                                <button 
                                    onClick={() => onReview(jobRequest)}
                                    className="bg-red-500 text-white font-bold py-2 px-6 rounded-lg hover:bg-red-600 transition duration-300 shadow-md flex items-center justify-center gap-2"
                                >
                                    <Icon name="star" className="h-4 w-4" />
                                    Debug: Force Review
                                </button>
                            ...
                            {/* Test button to simulate payment release */}
                            {jobRequest.status === JobStatus.Completed && jobRequest.paymentStatus === 'unpaid' && (
                                <button 
                                    onClick={() => {
                                        // Simulate changing payment status for testing
                                        const updatedJob = { ...jobRequest, paymentStatus: 'released' as const };
                                        setJobRequest(updatedJob);
                                    }}
                                    className="bg-green-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-green-600 transition duration-300 shadow-md flex items-center justify-center gap-2"
                                >
                                    <Icon name="check-circle" className="h-4 w-4" />
                            ...
                            {!jobRequest.isReviewed && jobRequest.paymentStatus === 'unpaid' && (
                                <div className="flex items-center justify-center gap-2 text-amber-600 bg-amber-100 py-2 px-4 rounded-lg">
                                    <Icon name="clock" className="h-4 w-4" />
                                    <span className="font-semibold">Payment required before leaving review</span>
                                </div>
                            )}
              ...
              {jobRequest.bids.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {jobRequest.bids.map(bid => {
                    ...
                    if (!artisan) return <div key={bid.id} className="bg-white rounded-lg shadow-md p-4 animate-pulse h-64"></div>; // Skeleton loader
                    return (
                      <ArtisanBidCard
                        key={bid.id}
                        job={jobRequest}
